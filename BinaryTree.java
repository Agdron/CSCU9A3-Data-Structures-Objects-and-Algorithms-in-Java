...
public class BinaryTree {
	...
	protected void walkTree(Node n, StringBuffer sb) {
		...
		//The following method is breadthFirst traversal
		List<Node> list = new LinkedList<Node>();
		list.add(n);
		//This is where the fun begins
		while(!list.isEmpty()) { //While list is not empty
			Node node = list.remove(0);
			sb.append(node.getNodeObjectName() + ", "); //Adds n onto the string

			if (node.hasLeft()) list.add(node.left());
			if (node.hasRight()) list.add(node.right());
		}
	}
	...
	protected Customer find(String name, Node n) {
		...
		int order = name.compareTo(n.getNodeObjectName()); //This compares the name we're searching for to the names in the tree
		//From there, this function will search through the tree depending on whether the name is found
		if (order < 0){
			return find(name, n.left()); //Search the left side of the tree using recursion
		}
		else if (order == 0){
			return n.getNodeObject(); //return if found
		}
		else{
			return find(name, n.right()); //Search the right side of the tree using recursion
		}
	}
